...
export const useFirebaseSubtasks = (userId?: string, taskId?: string) => {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        console.error('Error setting up subtasks listener:', err);
        setError(`Failed to setup subtasks connection: ${err.message}`);
        setLoading(false);
      }
    };
  ...
  }, [userId, taskId, loadSubtasksFromLocalStorage]);
